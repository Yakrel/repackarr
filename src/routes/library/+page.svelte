	...
	async function forceAddRelease(skip: SkipInfo) {
		...
		try {
			...
			if (result.success) {
				...
				if (skippedReleases.length === 0) showSkippedFor = null;
				await invalidateAll();
			} else {
							...
							<td class="px-4 py-3">
								<div class="flex items-center gap-3">
									{#if game.coverUrl}
										...
										/>
										<div
											class="w-16 h-24 rounded bg-slate-700 items-center justify-center text-xs text-slate-500 shrink-0 hidden"
										>
											?
										</div>
									{:else}
										<div
											class="w-16 h-24 rounded bg-slate-700 flex items-center justify-center text-xs text-slate-500 shrink-0"
										>
											?
										</div>
									{/if}
		...
		<div class="bg-gradient-to-br from-slate-800/95 to-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-700/50 w-full max-w-lg shadow-2xl">
			...
			>
				...
				<div>
					...
					<p class="text-[10px] text-amber-400/80 mt-1.5 italic flex items-center gap-1">
						<svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
						</svg>
						Tip: Use the base game name (e.g., "Elden Ring"). Including editions like "Deluxe" or "Shadow of the Erdtree" may cause searches to fail.
					</p>
				</div>
