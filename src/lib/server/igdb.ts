...
export async function getGameMetadata(gameName: string): Promise<GameMetadata | null> {
	...
	try {
		...
		const query = `fields name, cover.image_id, category, platforms, external_games.*; search "${gameName}"; limit 10;`;
		...
		for (const { game: candidate } of rankedResults.filter(r => r.score >= 60)) {
			for (const ext of (candidate.external_games || [])) {
 ...
 * Uses IGDB's dedicated 'search' engine for better relevance and typo tolerance.
 */
export async function searchGamesAutocomplete(
	...
	try {
		...
		const body = `search "${escaped}"; fields name, first_release_date, platforms; where ${platformFilter}; limit 10;`;
